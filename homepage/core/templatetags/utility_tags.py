...
def friendly_date(date, include_time=False):
  ...
  """
    ...
    if delta < timedelta(seconds=60):
        ...
        msg = ngettext("%(minutes)s minute ago", "%(minutes)s min ago", minutes) % {
            "minutes": minutes
        }
    elif delta < timedelta(days=1):
        hours = int(delta.seconds / 60 / 60)
        msg = ngettext("%(hours)s hour ago", "%(hours)s hours ago", hours) % {
            "hours": hours
        }
    elif delta < timedelta(days=7):
        msg = ngettext("%(days)s day ago", "%(days)s days ago", delta.days) % {
            "days": delta.days
        }
    elif delta < timedelta(days=31):
        weeks = int(delta.days / 7)
        msg = ngettext("%(weeks)s week ago", "%(weeks)s weeks ago", weeks) % {
            "weeks": weeks
        }
    elif delta < timedelta(days=365):
        months = int(delta.days / 31)
        msg = ngettext("%(months)s month ago", "%(months)s months ago", months) % {
            "months": months
        }
    else:
        years = int(delta.days / 365)
        msg = ngettext("%(years)s year ago", "%(years)s years ago", years) % {
            "years": years
        }
...
def urlize_twitter(text):
    return mark_safe(
        re.sub(
            ...
            re.sub(
                r"#([a-zA-Z0-9_-]*)",
                r'<a href="http://twitter.com/#search?q=\1">#\1</a>',
                text,
            ),
        )
    )
