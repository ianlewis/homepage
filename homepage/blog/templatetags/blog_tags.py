...
from homepage.core.templatetags.utility_tags import rst_to_html, md_to_html
...
pygments_directive.options = OPTIONS = {"startinline": flag}  # For PHP
...
def lightbox_directive(
    ...
    html = (
        '<a title="%(title)s" rel="lightbox" href="%(img_href)s"><img %(classes)s src="%(thumb_href)s" title="%(title)s" alt="%(alt)s"/></a>'
        % {
...
def to_lead(obj, max_len=None):
    if obj.lead:
        ...
        try:
            ...
            logger.error('HTML Parse error: "%s" for text "%s"' % (e, html))
            return ""
...
def timestamp(value, arg=None):
    ...
    """
    return datefilter(
        datetime.utcfromtimestamp(value).replace(tzinfo=timezone.utc), arg
    )
