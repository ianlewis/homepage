...
urlpatterns = patterns('django.views.generic.simple',
    ...
    url(r'^(?P<locale>jp|en)/(?P<tag>[^/]+);$', 'redirect_to', {'url': '/%(locale)s/tag/%(tag)s'}, name='old_blog_tag_page'),
)
...
urlpatterns += patterns('homepage.blog.views',
    ...
    url(r'^(?P<locale>jp|en)/tag/(?P<tag>.+)$', 'tag_page', name='blog_tag_page'),
        
    url(r'^(?P<locale>jp|en)/(?P<slug>[^/]+)/?$', 'blog_detail', name='blog_detail'),
